...
<div>
	...
	<div class="pagetitle">
		<h1>旅行社</h1>
		<div class="btn-group">
			<a id="user-add" href="javascript:void(0);" class="btn btn-primary">增加旅行社</a>
		</div>
		<div class="clearfix"></div>
		<hr>
	</div>
				...
				<tbody>
					...
					<tr>
						...
						<td class="action-table">
							<a href="javascript:void(0);" data-url="${ContextPath}/user/agency/update.do?id=${user.id}"
								class="page-load-btn safe" data-target="#mb" title="修改"><i class="icon-edit"></i></a>
							<a href="javascript:void(0);" data-url="${ContextPath}/user/agency/delete.html?id=${user.id}"
...
$('#user-add').click(function(){
	bootbox.prompt("旅行社名称", "取消", "确定", function(result) {
		...
		if (name.length) {
			$.post('${ContextPath}/user/add.do', {
				...
				hash['u'] = '${ContextPath}/user/agency/update.do?id=' + response;
