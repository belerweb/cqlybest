		...
		<div class="tab-content" style="margin: 45px 0;">
			<div id="i-island" class="tab-pane active">
				...
				<div class="row-fluid">
					...
					<div class="span12"><img alt="${island.zhName}" src="http://${ImageServer}/${img.qiniuKey}?imageView/1/w/480/h/200" width="100%"></div>
				</div>
				</#if>
				<div class="p-l-r-5">
					<!-- 海岛描述 -->
			...
			<div class="p-l-r-5">
				...
				<div class="row-fluid m-b-5">
					...
					<#list imgs as img>
					<div class="span${12/imgs?size}"><img alt="${room.zhName}" src="http://${ImageServer}/${img.qiniuKey}?imageView/1/w/${480/imgs?size}/h/${(400/imgs?size)?string('0')}" width="100%"></div>
					</#list>
				</div>
			...
			<div class="p-l-r-5">
				...
				<div class="row-fluid m-b-5">
					...
					<#list imgs as img>
					<div class="span${12/imgs?size}"><img alt="${dining.zhName}" src="http://${ImageServer}/${img.qiniuKey}?${480/imgs?size}/${(400/imgs?size)?string('0')}" width="100%"></div>
					</#list>
				</div>
				<div class="alert alert-success">图片已优化，适合移动设备访问，省流量，请放心浏览。欢迎使用电脑访问我们网站(${Settings.basic.siteUrl!})浏览精美图片信息。<br>如须出行，欢迎致电<a href="tel:${Settings.basic.hotline!}">${Settings.basic.hotline!}</a>咨询。</div>
				</#if>
