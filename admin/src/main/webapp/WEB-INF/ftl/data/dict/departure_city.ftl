...
<div class="grid-content overflow">
	<form id="main-content-form" action="${ContextPath}/data/dict/add_departure_city.html" method="post" class="form-horizontal">
	<table id="main-list-table" class="table table-striped">
		...
		<tbody>
			...
			<tr>
				<td>${dict.name!}</td>
				<td class="action-table">
					<a href="javascript:void(0);" data-url="${ContextPath}/data/dict/delete_departure_city.html?id=${dict.id}"
		...
		$form.ajaxSubmit({
			success : function(response) {
				cqlybest.success(null, null, function(){
					$('#dict-main').load('${ContextPath}/data/dict/departure_city.html');
				});
			},
			error : function() {
				cqlybest.error();
