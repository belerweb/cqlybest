		...
		<form id="ajax-form" method="POST" action="${ContextPath}/maldives/flight/update.do" class="form-horizontal">
			...
			<div class="control-group">
				...
				<div class="controls">
					...
					<input id="arrivals-time-day" type="text" class="input-mini" value="<#if (flight.arrivalsTime)?has_content>${flight.arrivalsTime?string('d')}<#else>0</#if>" />
					<span class="help-inline"><strong>：</strong></span>
					...
					<span class="help-inline">当地时间（第几天几点几分，0表示当天，1表示第二天）</span>
				</div>
			</div>
			<div class="control-group">
...
$('#ajax-form').submit(function() {
	...
	var arrivalsTime = $('#arrivals-time-hour').val()*86400000 + $('#arrivals-time-hour').val()*3600000 + $('#arrivals-time-minute').val()*60000;
...
$('#arrivals-time-day').ace_spinner({min:0, max:10, value:$('#arrivals-time-day').val()});
