<div id="product-detail-tab" class="tab-pane">
	<#assign PMURL="${ContextPath}/maldives/product/room/update.do">
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<#list product.maldivesDetails as detail>
			<tr>
				<td>
...
$('#product-detail-tab button.add').click(function(){
	...
	bootbox.prompt("行程名称", "取消", "确定", function(result) {
		...
		if (name.length) {
			$.post('${ContextPath}/maldives/product/room/add.do', {
...
$('#product-detail-tab button.delete').click(function() {
	...
	bootbox.confirm('确认删除行程', '取消', '确认', function(result) {
		if (result) {
			$.post('${ContextPath}/maldives/product/room/delete.do', {
				id: id
			}).done(function(){
				row.next().remove();
				row.remove();
...
$('#product-detail-tab a[data-name=island]').on('save', function(){
	...
	select2: {
		...
		ajax: {
			url: '${ContextPath}/maldives/island/ajax.do',
...
$('#product-detail-tab a[data-name=room]').each(function(i, obj){
	...
	$(obj).editable({
		select2: {
			...
			ajax: {
				url: '${ContextPath}/maldives/room/ajax.do?islandId='+islandId,
