<div id="hotel-room-tab" class="tab-pane">
	<div class="text-right"><button id="hotel-room-add" type="button" class="btn btn-primary">添加</button></div>
	<div id="hotel-room-accordion-container" style="margin-top:5px;">
		<#assign rooms=hotel.rooms>
		<#include "update_room_accordion.ftl">
	</div>
</div>

<script type="text/javascript">
$('#hotel-room-add').click(function(){
	...
	var dialog = bootbox.dialog(form.join(''), [{
		...
		callback: function(){
			...
			$.post('${ContextPath}/mauritius/room/add.do', {
				hotelId: '${hotel.id}',
				zhName: zhName,
				enName: enName
			}).done(function(data){
				$('#mb').load('${ContextPath}/mauritius/update.do?id=${hotel.id}', function() {
					$('#hotel-update-tabs a[data-target="#hotel-room-tab"]').tab('show');
				});
			}).fail(function() {
				cqlybest.error();
			});
