<div id="product-traffic-tab" class="tab-pane">
	...
	<table class="table table-striped table-bordered table-hover">
		...
		<tbody>
			<#assign PTURL="${ContextPath}/seychelles/product/transportation/update.do" />
...
$('#product-traffic-tab button.add').click(function(){
	...
	var dialog = bootbox.dialog(form.join(''), [{
		...
		callback: function(){
			...
			$.post('${ContextPath}/seychelles/product/transportation/add.do', {
...
$('#product-traffic-tab button.delete').on('click', function() {
	...
	bootbox.confirm('确认删除交通？', '取消', '确认', function(result) {
		if (result) {
			$.post('${ContextPath}/seychelles/product/transportation/delete.do', {
				id: id
			}).done(function(){
				el.remove();
			}).fail(function() {
