...
<div id="page-content" class="clearfix">
	<div class="row-fluid">
		<h3 class="header smaller lighter blue">塞舌尔海岛/酒店
...
$('#page-content button[data-action=add]').click(function(){
	...
	var dialog = bootbox.dialog(form.join(''), [{
		...
		callback: function(){
			...
			$.post('${ContextPath}/seychelles/add.do', {
				zhName: zhName,
				enName: enName
			}).done(function(data){
				cqlybest.go('#main-content', '${ContextPath}/seychelles/update.do?id=' + data);
			}).fail(function() {
				cqlybest.error();
			});
		}
...
$('#page-content table').dataTable({
	...
	fnServerData: function (sSource, aoData, fnCallback, oSettings) {
		...
		cqlybest.go('#main-content', '${ContextPath}/seychelles/list.do?' + $.param(q));
	}
});
$('#page-content button[data-action=preview]').click(function(){
	<#if (settings.basic.siteUrl)?has_content>
	window.open('${settings.basic.siteUrl}/seychelles/' + $(this).data('id')  + '.html');
	<#else>
	cqlybest.error('请先配置网站地址。');
	</#if>
});
$('#page-content button[data-action=edit]').click(function(){
	cqlybest.go('#main-content', '${ContextPath}/seychelles/update.do?id=' + $(this).data('id'));
});
