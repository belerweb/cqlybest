...
<div id="island-dining-accordion" class="accordion">
	...
	<div class="accordion-group">
		...
		<#assign diningUrl="${ContextPath}/seychelles/dining/update.do" />
		<div id="island-dining-accordion-${dining_index}" class="accordion-body collapse <#if dining_index==0>in</#if>" style="overflow:visible;">
					...
					<div class="span12">
						<div class="control-group">
							...
							<div class="controls image-gallery">
								<button type="button" class="btn btn-primary btn-mini action action-add picture" data-name="diningId" data-value="${diningId}" data-url="${ContextPath}/seychelles/dining/picture/add.do">添加</button>
									...
									<#list dining.pictures as image>
										<li class="span3">
											<div class="thumbnail">
												...
												<div class="caption">
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="title" data-type="text" data-value="${image.title!}" data-url="${ContextPath}/seychelles/dining/picture/update.do"></a></p>
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="description" data-type="textarea" data-url="${ContextPath}/seychelles/dining/picture/update.do">${image.description!?html}</a></p>
													<button class="delete btn btn-danger" type="button" data-id="${image.id}" data-url="${ContextPath}/seychelles/dining/picture/delete.do">刪除</button>
												</div>
											</div>
										</li>
										<#if image_index%4==3>
...
$('#island-dining-accordion .accordion-heading button.delete').click(function(e) {
	...
	bootbox.confirm('确认删除餐饮设施?', '取消', '确认', function(result) {
		if (result) {
			$.post('${ContextPath}/seychelles/dining/delete.do', {
				id: id
			}).done(function(){
				el.remove();
			}).fail(function() {
