...
<div id="island-room-accordion" class="accordion">
	...
	<div class="accordion-group">
		...
		<#assign roomUrl="${ContextPath}/seychelles/room/update.do" />
					...
					<div class="span12">
						<div class="control-group">
							...
							<div class="controls image-gallery">
								<button type="button" class="btn btn-primary btn-mini action action-add picture" data-name="roomId" data-value="${roomId}" data-url="${ContextPath}/seychelles/room/picture/add.do">添加</button>
									...
									<#list room.pictures as image>
										<li class="span3">
											<div class="thumbnail">
												...
												<div class="caption">
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="title" data-type="text" data-value="${image.title!}" data-url="${ContextPath}/seychelles/room/picture/update.do"></a></p>
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="description" data-type="textarea" data-url="${ContextPath}/seychelles/room/picture/update.do">${image.description!?html}</a></p>
													<button class="delete btn btn-danger" type="button" data-id="${image.id}" data-url="${ContextPath}/seychelles/room/picture/delete.do">刪除</button>
												</div>
											</div>
										</li>
										<#if image_index%4==3>
...
$('#island-room-accordion .accordion-heading button.delete').click(function(e) {
	...
	bootbox.confirm('确认删除房型?', '取消', '确认', function(result) {
		if (result) {
			$.post('${ContextPath}/seychelles/room/delete.do', {
				id: id
			}).done(function(){
				el.remove();
			}).fail(function() {
