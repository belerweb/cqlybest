...
<div id="hotel-room-accordion" class="accordion">
	<#if hotel.rooms?has_content>
	<#list hotel.rooms as room>
	<div class="accordion-group">
		<div class="accordion-heading" style="position:relative;">
			<a class="accordion-toggle" data-toggle="collapse" data-parent="#hotel-room-accordion" href="#hotel-room-accordion-${room_index}">
		...
		<#assign roomUrl="${ContextPath}/mauritius/room/update.do" />
		<div id="hotel-room-accordion-${room_index}" class="accordion-body collapse <#if room_index==0>in</#if>" style="overflow:visible;">
					...
					<div class="span12">
						<div class="control-group">
							...
							<div class="controls image-gallery">
								<button type="button" class="btn btn-primary btn-mini action action-add picture" data-name="roomId" data-value="${roomId}" data-url="${ContextPath}/mauritius/room/picture/add.do">添加</button>
									...
									<#list room.pictures as image>
										<li class="span3">
											<div class="thumbnail">
												...
												<div class="caption">
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="title" data-type="text" data-value="${image.title!}" data-url="${ContextPath}/mauritius/room/picture/update.do"></a></p>
													<p><a href="#" class="editable" data-pk="${image_index}|${image.id}" data-name="description" data-type="textarea" data-url="${ContextPath}/mauritius/room/picture/update.do">${image.description!?html}</a></p>
													<button class="delete btn btn-danger" type="button" data-id="${image.id}" data-url="${ContextPath}/mauritius/room/picture/delete.do">刪除</button>
												</div>
											</div>
										</li>
										<#if image_index%4==3>
...
$('#hotel-room-accordion .accordion-heading button.delete').click(function(e) {
	var id = $(this).attr('data-id');
	var el = $($(this).parents('.accordion-group').get(0));
	bootbox.confirm('确认删除房型?', '取消', '确认', function(result) {
		if (result) {
			$.post('${ContextPath}/mauritius/room/delete.do', {
				id: id
			}).done(function(){
				el.remove();
			}).fail(function() {
...
$('#hotel-room-tab a[data-name="containPool"]').editable({
	prepend: '',
